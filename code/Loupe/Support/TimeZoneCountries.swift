//
//  TimeZoneCountries.swift
//  Loupe
//
//  Maps IANA time zone identifiers to ISO 3166-1 alpha-2 country codes.
//  Sourced from the public `zone.tab` distribution; trimmed to entries
//  the iOS time zone database actually exposes. Used by the simplified
//  summary to detect the "different timezone than region" travel hint.
//

import Foundation

nonisolated enum TimeZoneCountries {
    static func countryCode(for identifier: String) -> String? {
        if let direct = table[identifier] { return direct }
        // Some platforms hand back link identifiers (e.g. "US/Pacific");
        // resolve to the canonical name via TimeZone before giving up.
        if let canonical = TimeZone(identifier: identifier)?.identifier,
           canonical != identifier,
           let mapped = table[canonical] {
            return mapped
        }
        return nil
    }
    ...
    private static let table: [String: String] = [
        ...
        "America/Lima": "PE", "America/Los_Angeles": "US", "America/Lower_Princes": "SX",
        ...
        "Europe/Kirov": "RU", "Europe/Kyiv": "UA", "Europe/Lisbon": "PT", "Europe/Ljubljana": "SI",
